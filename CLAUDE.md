...
- `main` et `staging` sont protégées par ruleset GitHub : suppression et force-push bloqués, les 5 checks CI sont requis, 1 review humaine requise (dismiss automatique si nouveau push), threads de conversation à résoudre avant merge, et une review Copilot automatique se déclenche à l'ouverture de PR et à chaque push.
...
- Modifier du code applicatif dans une branche dédiée, committer, ouvrir une PR (jamais push direct sur `main`/`staging` — de toute façon bloqué par les rulesets).
